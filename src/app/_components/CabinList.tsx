...
async function CabinList({capacity}:CabinListProps ) {
  ...
  if(capacity == "small") displayCabinList = cabins.filter((cabin: any) => {return(
       cabin.maxCapacity < 3
  )})
  if(capacity == "medium") displayCabinList = cabins.filter((cabin: any) => {return(
       cabin.maxCapacity >= 4 && cabin.maxCapacity <= 7
  )})
  if(capacity == "large") displayCabinList = cabins.filter((cabin: any) => {return(
    cabin.maxCapacity >= 8
  )})
